...
- IM Fell English SC for display, IM Fell English (italic) for Poe's lines; all type shudders by analyser amplitude. The HUD score alone is set in **Cinzel 600** (30px desktop / 25px mobile, bone at 80%): IM Fell's figures are old-style, so a lone `0` rendered as a small ring at the top right of every first frame. Cinzel's lining figures match Fell SC's cap height and weight at that size; Marcellus SC was tried and rejected because its zero is a wide round O that reads as a ring again. The labels under it (`first night` / `best …`, `n% to dawn`, 16/14px) stay in IM Fell.
...
## Mechanic
- The heart holds flat at 54 BPM for the first six beats (a settling intro, ~6.7 s, so a player who locks onto the opening interval is not already drifting by beat four), then climbs on the original `u^1.2` curve compressed into the remaining night, reaching ~152 BPM at dawn (76 s) as before; the night is now 118 beats instead of 124. Perfect is ±90 ms on a first night (no score on record), easing to ±80 ms by dawn; once a score is on the books it is ±75 ms. Good is ±150 ms. Perfects restore composure, misses and off-beat taps drain it, a tell costs a lot. Composure also drains on its own, faster as the night wears on.
